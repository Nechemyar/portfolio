...
test('About section markup follows the Stevia-style chat and info panel structure', () => {
  ...
  assert.match(html, /class="about-chat"[^>]*data-chat-scroll/);
  ...
  assert.match(html, /class="about-chat__typing"/);
...
test('About CSS uses full-screen chat column and fixed info panel layout', () => {
  ...
  assert.match(scss, /\.about-chat\s*\{[\s\S]*overflow-y:\s*auto/);
  assert.match(scss, /\.about-chat\s*\{[\s\S]*padding-top:\s*clamp\(2\.5rem,\s*8vh,\s*6rem\)/);
  assert.match(scss, /\.about-chat__list\s*\{[\s\S]*position:\s*relative/);
  ...
  assert.match(scss, /\.about-chat__typing\s*\{[\s\S]*position:\s*sticky/);
  assert.match(scss, /\.about__content\s*\{[\s\S]*height:\s*100vh/);
  assert.match(scss, /\.about-infos\s*\{[\s\S]*grid-template-rows:\s*auto 1fr auto/);
  assert.match(scss, /\.about-infos__skills\s*\{/);
  assert.match(scss, /\.about-infos__skill\s*\{[\s\S]*font-size:\s*clamp\(1\.35rem,\s*3\.2vw,\s*4\.2rem\)/);
  assert.match(scss, /\.about-links\s*\{[\s\S]*display:\s*grid/);
  assert.match(scss, /@media \(max-width: 640px\)/);
...
test('AboutReveal module pins desktop section and scrolls the chat list', () => {
  ...
  assert.match(module, /this\.typingIndicator/);
  assert.match(module, /chatDistance/);
  assert.match(module, /_runTypingSequence/);
  assert.match(module, /about-chat--is-typing/);
  assert.match(module, /about-chat__el--is-revealed/);
  assert.match(module, /scrollTo/);
